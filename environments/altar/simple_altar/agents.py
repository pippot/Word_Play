from environments.altar.simple_altar.actions import (
...
class Constant_Harvest_Agent(Constant_Strategy_Agent):
	exposed_actions = AGENT_EXPOSED_ACTIONS
	actions_on_self = AGENT_ACTIONS_ON_SELF
	...
	def history_to_str(self, obs_history, my_action_history) -> str:
		...
		for round, (obs, my_action) in self.get_history_enumerator(obs_history, my_action_history):
			if not obs.other_agent_actions_last_step:
				all_agent_actions = ['None' for agent in obs.other_all_agent_names] + [my_action]
			else:
				all_agent_actions = obs.other_agent_actions_last_step + [my_action]

			all_agent_names = obs.other_all_agent_names + ['Me']
			all_actions_str = ''
...
# Possible Actions:
{format_possible_actions(obs_history[-1].possible_actions)}
