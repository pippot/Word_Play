...
def run_overcooked_example(
    *,
    ...
    renderer = PygameRenderer(layout=ExampleKitchenLayoutAdapter(), tile_size=tile_size)
    env_type = SinglePlayerOvercookedEnv if single_player else ExampleOvercookedEnv

    def reset_factory() -> ExampleOvercookedEnv:
        return env_type(renderer=renderer)

    def step_builder(env: ExampleOvercookedEnv):
...
def run_overcooked_env(**kwargs) -> str | None:
    """Backward-compatible alias for older imports."""
    return run_overcooked_example(**kwargs)
