...
MODEL_KEY = "predator_prey__random_forest"
...
def run_exp(agent_count: int = DEFAULT_NUM_PLAYERS, policy: str = "random", model_name: str = "openai/gpt-4o-mini"):
    ...
    for agent_id, (role, (x, y)) in enumerate(spawn_assignments, start=1):
        if policy == "llm":
            ...
            agent_policy = LLM_Action_And_Communication_Policy(
                model_key=MODEL_KEY,
                system_prompt=(
                    f"You are Player {agent_id}, a {role}, in Predator Prey Random Forest. "
    ...
    env = Simple_2D_Grid_World(
        description="Predator Prey: Random Forest, adapted from Melting Pot.",
        entities=entities,
