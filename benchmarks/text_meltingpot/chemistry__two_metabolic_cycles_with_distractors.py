...
MODEL_KEY = "chemistry__two_metabolic_cycles_with_distractors"
...
def run_exp(agent_count: int, policy: str, model_name: str):
    ...
    entity_tileset = {
        ...
        "x": {
            ...
            "components": [
                ...
                Renderable(sprite_path="src/items/consumables/magic/purple_red_potion.png", z_index=4),
            ],
        },
        "1": {
    ...
    for agent_id, (x, y) in enumerate(spawn_positions[:agent_count], start=1):
        role_prompt = (
            "Your job is blue/X: carry energy to Blue Compounds, make Molecule X, then bring X toward Y. "
            if agent_id <= agent_count // 2
            else "Your job is green/Y: carry energy to Green Compounds, make Molecule Y, then bring Y toward X. "
        )
        agent_policy = (
            LLM_Action_And_Communication_Policy(
                model_key=MODEL_KEY,
                system_prompt=(
                    "You are playing Chemistry: Two Metabolic Cycles With Distractors. "
                    ...
                    "If your inventory is empty, prefer energy, food, x, or y; distractors are only a fallback. "
                    "If holding energy, move onto a Blue or Green Compound; those compounds are cycle sites. "
                    ...
                    "Distractors pay only +0.1 while held; x+y pays +10. "
    ...
    env = Simple_2D_Grid_World(
        description="Chemistry: Two Metabolic Cycles With Distractors, adapted from MeltingPot.",
        entities=entities,
