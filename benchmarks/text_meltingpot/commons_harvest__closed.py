...
def run_exp(agent_count: int = 7, policy: str = "random", model_name: str = "openai/gpt-4o-mini"):
    ...
    if policy == "llm":
        LLM_MODEL_REGISTRY.unload("commons_harvest__closed")
    ...
    for agent_id, (x, y) in enumerate(spawn_positions[:agent_count], start=1):
        agent_policy = (
            LLM_Action_And_Communication_Policy(
                model_key="commons_harvest__closed",
                system_prompt=(
                    f"You are Player {agent_id} in Commons Harvest: Closed. "
    ...
    env = Simple_2D_Grid_World(
        description="Commons Harvest: Closed, adapted from MeltingPot.",
        entities=entities,
