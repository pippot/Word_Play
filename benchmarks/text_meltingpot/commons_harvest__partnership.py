...
def run_exp(agent_count: int = DEFAULT_NUM_PLAYERS, policy: str = "random", model_name: str = "openai/gpt-4o-mini"):
    ...
    if policy == "llm":
        LLM_MODEL_REGISTRY.unload("commons_harvest__partnership")
    ...
    entity_tileset = {
        ...
        "A": {
            "name": "Apple",
            "components": [
                Commons_Apple_Patch(apple_regrowth_probs=[0.0, 0.001, 0.005, 0.025]),
        ...
        "I": {
            ...
            "components": [
                ...
                Renderable(sprite_path="src/world_tiles/indoors/floors/white_grid_floor.png", z_index=1),
            ],
        },
    }
    ...
    for agent_id, (x, y) in enumerate(spawn_positions[:agent_count], start=1):
        agent_policy = (
            LLM_Action_And_Communication_Policy(
                model_key="commons_harvest__partnership",
                system_prompt=(
                    f"You are Player {agent_id} in Commons Harvest: Partnership. "
        ...
        entities.append(
            Entity(
                ...
                components=[
                    agent_policy,
                    Cooldown(cooldowns={"zap": normalized_steps(1)}),
                    Freezable(default_duration=normalized_steps(100)),
                    Collidable(collidable_tags=["wall"]),
    ...
    env = Simple_2D_Grid_World(
        description="Commons Harvest: Partnership, adapted from MeltingPot.",
        entities=entities,
