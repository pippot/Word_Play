...
def run_exp(agent_count: int = DEFAULT_NUM_PLAYERS, policy: str = "random", model_name: str = "openai/gpt-4o-mini"):
    ...
    for agent_id, (x, y) in enumerate(spawn_positions[:agent_count], start=1):
        if policy == "llm":
            ...
            agent_policy = LLM_Action_And_Communication_Policy(
                model_key=MODEL_KEY,
                system_prompt=(
                    f"You are Player {agent_id} in Boat Race. Pair up, board a boat, and coordinate Paddle actions. "
                    "Two players paddling in the same step advances race progress; flailing can sometimes help."
                ),
    ...
    env = Simple_2D_Grid_World(
        description="Boat Race, adapted from Melting Pot.",
        entities=entities,
