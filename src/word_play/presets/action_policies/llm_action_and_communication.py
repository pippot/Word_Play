...
class LLM_Action_And_Communication_Policy(Agent_Policy, Communication_Policy):
    ...
    def select_actions_batched(
        cls,
        policies: Sequence["LLM_Action_And_Communication_Policy"],
        observations: Sequence[Observation],
        *,
        max_workers: int | None = None,
    ) -> list[tuple[Action_Selection, dict]]:
        if len(policies) != len(observations):
            raise ValueError("policies and observations must have the same length.")
        if not policies:
            return []

        reasonings: list[str | None] = [None] * len(policies)
        reasoning_indices = [idx for idx, policy in enumerate(policies) if policy.use_chain_of_thought]
        if reasoning_indices:
            reasoning_prompts = [
                policies[idx]._with_system(policies[idx]._reasoning_prompt(observations[idx]))
                for idx in reasoning_indices
            ]
            reasoning_outputs = cls._generate_text_batch_grouped(
                [policies[idx] for idx in reasoning_indices],
                reasoning_prompts,
                config_attr="reasoning_generation_config",
                max_tokens_attr="reasoning_max_new_tokens",
                max_workers=max_workers,
            )
            for idx, reasoning in zip(reasoning_indices, reasoning_outputs):
                reasonings[idx] = reasoning.strip()

        selection_prompts = [
            policy._selection_prompt(observation, reasoning)
            for policy, observation, reasoning in zip(policies, observations, reasonings)
        ]
        pending = list(range(len(policies)))
        results: list[tuple[Action_Selection, dict] | None] = [None] * len(policies)
        last_raws: list[str | None] = [None] * len(policies)
        last_errors: list[Exception | None] = [None] * len(policies)
        max_attempts = max(policy.MAX_ATTEMPTS for policy in policies)

        for attempt in range(max_attempts):
            active_indices = [idx for idx in pending if attempt < policies[idx].MAX_ATTEMPTS]
            if not active_indices:
                break

            raw_outputs = cls._generate_text_batch_grouped(
                [policies[idx] for idx in active_indices],
                [policies[idx]._with_system(selection_prompts[idx]) for idx in active_indices],
                config_attr="action_generation_config",
                max_tokens_attr="action_max_new_tokens",
                max_workers=max_workers,
            )

            next_pending: list[int] = []
            for idx, raw in zip(active_indices, raw_outputs):
                policy = policies[idx]
                observation = observations[idx]
                last_raws[idx] = raw
                try:
                    action_selection = policy._parse_selection(raw, observation)
                    policy._record_observation(observation)
                    info = {
                        "raw_response": raw,
                    ...
                    policy._record_last_selection(action_selection, info)
                    results[idx] = (action_selection, info)
                except Exception as exc:
                    last_errors[idx] = exc
                    selection_prompts[idx] = policy._retry_prompt(selection_prompts[idx], raw, str(exc))
                    next_pending.append(idx)

            pending = [idx for idx in pending if idx not in active_indices] + next_pending
            if not pending:
                break
        ...
        if pending:
            idx = pending[0]
            raise RuntimeError(
                f"LLM failed to produce a valid action for batched agent index {idx} "
                f"after {policies[idx].MAX_ATTEMPTS} attempts. "
                f"Last error: {last_errors[idx]}\n"
                f"Last raw response:\n{last_raws[idx]}"
            )

        resolved_results: list[tuple[Action_Selection, dict]] = []
        for idx, result in enumerate(results):
            if result is None:
                raise RuntimeError(f"No batched action result was produced for agent index {idx}.")
            resolved_results.append(result)
        return resolved_results

    @staticmethod
    def _freeze_generation_config(value: Any) -> Any:
        if isinstance(value, Mapping):
            return tuple(
                sorted(
                    (
                        (key, LLM_Action_And_Communication_Policy._freeze_generation_config(val))
        ...
        if isinstance(value, (list, tuple)):
            return tuple(LLM_Action_And_Communication_Policy._freeze_generation_config(val) for val in value)
        if isinstance(value, set):
            return tuple(
                sorted(
                    (LLM_Action_And_Communication_Policy._freeze_generation_config(val) for val in value),
                    key=repr,
                )
            )
        try:
    ...
    @classmethod
    def _generate_text_batch_grouped(
        cls,
        policies: Sequence["LLM_Action_And_Communication_Policy"],
        ...
        *,
        ...
        grouped_indices: dict[tuple[Model, Any, int | None], list[int]] = {}
        for idx, policy in enumerate(policies):
            generation_config = getattr(policy, config_attr)
            max_tokens = getattr(policy, max_tokens_attr)
            key = (policy.model, cls._freeze_generation_config(generation_config), max_tokens)
            grouped_indices.setdefault(key, []).append(idx)

        outputs: list[str | None] = [None] * len(prompts)
        for (model, _, max_tokens), indices in grouped_indices.items():
            generation_config = getattr(policies[indices[0]], config_attr)
            batch_outputs = model.generate_text_batch(
                [prompts[idx] for idx in indices],
                generation_config=generation_config,
                max_new_tokens=max_tokens,
                max_workers=max_workers,
            )
            if len(batch_outputs) != len(indices):
                raise RuntimeError(
                    f"Model returned {len(batch_outputs)} outputs for {len(indices)} batched prompts."
                )
            for idx, output in zip(indices, batch_outputs):
                outputs[idx] = output
